...
(async () => {
  ...
  const stars = card._stars(true);
  assert.equal((stars.match(/class="star(?: |")/g) || []).length, 76);
  assert.doesNotMatch(stars, /class="meteor"/);
  assert.equal(card._stars(true, {}, 100), '', 'overcast skies should not animate hidden stars');
  const fullMoonSky = card._stars(true, {altitude: 45, fraction: 100}, 0);
  const darkSky = card._stars(true, {altitude: -20, fraction: 100}, 0);
  const skyBrightness = sky => Number(sky.match(/--sky-visibility:([\d.]+)/)[1]);
  assert.ok(skyBrightness(fullMoonSky) < skyBrightness(darkSky), 'moonlight should wash out stars only above the horizon');
  assert.match(stars, /star bright/);
  assert.doesNotMatch(card._stars(false), /class="meteor"/);
  const rain = card._particles("rainy", 24, 90, "km\/h");
  assert.equal((rain.match(/particle rain/g) || []).length, 58);
  assert.match(rain, /particle rain far/);
  assert.match(rain, /particle rain near/);
  assert.match(rain, /rain-haze/);
  assert.match(rain, /rain-splash/);
  const downpour = card._particles("pouring", 35, 250, "km\/h");
  assert.equal((downpour.match(/particle rain/g) || []).length, 88);
  assert.match(card._sceneClass("pouring", false, 20), /rain-heavy/);
  assert.match(card._sceneClass("lightning-rainy", false, 20), /rain-storm/);
